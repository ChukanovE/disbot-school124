...
async def help(ctx):
    ...
    ctx.send(embed=emb)
    await
    ctx.author.send('Команды .bma и .bmt - аналогичны'
    ...
    await
    ctx.author.send(embed=emb)
    await
    ctx.author.send('Команда .bmt, в отличии от .bma - указывает автора'
...
async def clear(ctx, ammount=0):
    await
    ctx.channel.purge(limit=ammount + 1)
...
async def on_ready():
    print('Logged on as!')
    await
    client.change_presence(status=discord.Status.online, activity=discord.Game('Выставляю четвертные двойки'))
...
async def on_raw_reaction_add(payload):
    if payload.message_id == POST_ID:
        ...
        message = await
        channel.fetch_message(payload.message_id)  # получаем объект сообщения
